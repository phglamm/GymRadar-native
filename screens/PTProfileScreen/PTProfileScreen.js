      ...
      <LinearGradient colors={['#FF914D', '#ED2A46']} style={styles.gradientContainer}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>
            {pt.gender === 'Male' ? 'Nam' : 'Nữ'} - {pt.dob ? (new Date().getFullYear() - new Date(pt.dob).getFullYear()) + ' Tuổi' : ''}
          </Text>
        </View>

        <View style={styles.infoRow}>
          <MaterialCommunityIcons name="weight-lifter" size={20} color="#fff" />
          <Text style={styles.infoText}>{pt.goalTraining || 'Chưa có mục tiêu'}</Text>
        </View>

        <View style={styles.infoRow}>
          <MaterialCommunityIcons name="timer-sand" size={20} color="#fff" />
          <Text style={styles.infoText}>{pt.experience ? pt.experience + ' Năm Kinh Nghiệm' : 'Chưa có kinh nghiệm'}</Text>
        </View>
      </LinearGradient>
    </ScrollView>
  );
